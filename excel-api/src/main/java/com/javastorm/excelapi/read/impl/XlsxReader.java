...
/**
 * This class is intended for providing the functions for reading a xlsx file.
 * 
 * @author Hemant Kumar
...
public class XlsxReader implements Reader 
{
	...
	public String[][] readLastSheet(File file) throws Exception {
		return readSheet(file, (new XSSFWorkbook(new FileInputStream(file))).getNumberOfSheets()-1);
	}

	@Override
	public String[][] readLastSheet(String filePath) throws Exception {
		return readSheet(filePath, (new XSSFWorkbook(new FileInputStream(filePath))).getNumberOfSheets()-1);
	}

	@Override
	public String[][] readLastSheet(InputStream stream) throws Exception {
		return readSheet(stream, (new XSSFWorkbook(stream)).getNumberOfSheets()-1);
	}
}
